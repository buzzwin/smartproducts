...
from database.database import RepositoryFactory, init_database, get_db_session_context, db_config
...
async def import_csv_data(file_content: bytes):
    ...
    if db_config.is_sql:
        async with get_db_session_context() as session:
            if session:
                ...
                result = await _import_data(
                    file_content, product_repo, category_repo, scenario_repo,
...
async def _import_data(
    ...
    for row in rows:
        ...
        if parsed["type"] == "product":
            ...
            if existing:
                ...
                imported_products.append(product_name)
            current_category = None
    ...
    return {
        ...
        "imported_cost_items": imported_items,
...
async def import_csv(file: UploadFile = File(...)):
    """Import CSV file and populate database."""
    ...
    try:
        ...
        # Import data
        result = await import_csv_data(content)
