...
interface TaskListProps {
  ...
  initialFilterPhaseId?: string; // Optional - initial phase filter from parent
  initialFilterWorkstreamId?: string; // Optional - initial workstream filter from parent
  initialFilterFeatureId?: string; // Optional - initial feature filter from parent
...
export default function TaskList({
  ...
  initialFilterPhaseId,
  initialFilterWorkstreamId,
  initialFilterFeatureId,
  hideFilters = false,
  onUpdate,
}: TaskListProps) {
  ...
  const [filterModuleId, setFilterModuleId] = useState<string>(
    initialFilterModuleId || moduleId || ""
  );
  ...
  const [filterWorkstreamId, setFilterWorkstreamId] = useState<string>(
    initialFilterWorkstreamId || ""
  );
  ...
  useEffect(() => {
    if (initialFilterPhaseId !== undefined) {
      setFilterPhaseId(initialFilterPhaseId);
    }
  }, [initialFilterPhaseId]);

  useEffect(() => {
    if (initialFilterWorkstreamId !== undefined) {
      setFilterWorkstreamId(initialFilterWorkstreamId);
    }
  }, [initialFilterWorkstreamId]);

  useEffect(() => {
    if (initialFilterFeatureId !== undefined) {
      setFilterFeatureId(initialFilterFeatureId);
    }
  }, [initialFilterFeatureId]);
    ...
    try {
      ...
      const [
        ...
        (async () => {
          ...
          if (filterPhaseId) params.phase_id = filterPhaseId;
          if (filterWorkstreamId) params.workstream_id = filterWorkstreamId;
  ...
  const handleExportExcel = () => {
    const exportData = prepareTaskDataForExport(
      tasks,
      products,
      features,
      workstreams,
    ...
    const filename = `tasks-export-${
      new Date().toISOString().split("T")[0]
    }.xlsx`;
    exportTasksToExcel(exportData, filename);
  };
  ...
  const handleExportPDF = () => {
    ...
    exportTasksToPDF(exportData, filename);
  };
  ...
  return (
    <div className="card">
              ...
              return (
                <tr key={task.id}>
                    ...
                    task.depends_on_task_ids.length > 0 ? (
                      ...
                      >
                        {task.depends_on_task_ids.map((depTaskId) => (
                          ...
                          >
                            {getTaskTitle(depTaskId).substring(0, 20)}
                            {getTaskTitle(depTaskId).length > 20 ? "..." : ""}
                          </span>
                        ))}
                      </div>
                    ) : (
                  ...
                  <td style={{ textAlign: "right" }}>
                    {task.diagram_xml && task.diagram_xml.trim() && (
                      <button
                        onClick={() => setViewingDiagramTask(task)}
                        style={{
                          padding: "4px 12px",
                          fontSize: "12px",
                          backgroundColor: "#28a745",
                    ...
                    <button
                      ...
                      style={{
                        ...
                        fontSize: "12px",
                        backgroundColor:
                          deletingId === task.id ? "#ccc" : "#dc3545",
                        color: "#fff",
                        border: "none",
                        borderRadius: "4px",
                        cursor:
                          deletingId === task.id ? "not-allowed" : "pointer",
                      }}
                    >
      ...
      <Modal
        ...
        onClose={() => setViewingDiagramTask(null)}
