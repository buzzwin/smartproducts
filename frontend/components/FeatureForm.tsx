...
import type { Feature, Product, Problem, Module, CostClassification, Resource } from '@/types';
...
export default function FeatureForm({ feature, product, initialModuleId, onSuccess, onCancel }: FeatureFormProps) {
  const [name, setName] = useState(feature?.name || '');
  const [description, setDescription] = useState(feature?.description || '');
  const [owner, setOwner] = useState(feature?.owner || '');
  const [moduleId, setModuleId] = useState(feature?.module_id || initialModuleId || '');
  const [parentFeatureId, setParentFeatureId] = useState(feature?.parent_feature_id || '');
  const [order, setOrder] = useState<number>(feature?.order || 0);
  const [selectedProblemIds, setSelectedProblemIds] = useState<string[]>(feature?.problem_ids || []);
  const [expectedOutcomes, setExpectedOutcomes] = useState<string>(JSON.stringify(feature?.expected_outcomes || [], null, 2));
  const [status, setStatus] = useState(feature?.status || 'discovery');
  const [cost_classification, setCost_classification] = useState<CostClassification | ''>(feature?.cost_classification || '');
  ...
  const loadModulesAndProblems = async () => {
    try {
      setLoadingModules(true);
      console.log('Loading modules for product:', product.id);
  ...
  const loadFeatures = async () => {
    try {
      const params: { product_id: string; module_id?: string } = { product_id: product.id };
      if (moduleId) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const featureData = {
        ...
        parent_feature_id: parentFeatureId && parentFeatureId !== 'none' ? parentFeatureId : undefined,
        order: order || 0,
        problem_ids: selectedProblemIds.length > 0 ? selectedProblemIds : undefined,
        expected_outcomes: parsedOutcomes.length > 0 ? parsedOutcomes : undefined,
  ...
  return (
    <form onSubmit={handleSubmit} style={{ padding: 'clamp(16px, 4vw, 24px)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '16px' }}>
        <div>
          ...
          {loadingResources ? (
            ...
            <Select
              value={owner || 'none'}
              onValueChange={(value) => setOwner(value === 'none' ? '' : value)}
            >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '16px' }}>
              ...
              >
                ...
                <SelectContent style={{ zIndex: 9999 }}>
                  ...
                  {modules && modules.length > 0 ? (
                    ...
                    <SelectItem value="none" disabled>
                      No modules available (create modules in Management → Modules)
                    </SelectItem>
                  )}
                </SelectContent>
              </Select>
        ...
        <div>
          <Label htmlFor="parent-feature-select">Parent Feature (optional)</Label>
          <Select
            value={parentFeatureId || 'none'}
            onValueChange={(value) => setParentFeatureId(value === 'none' ? '' : value)}
